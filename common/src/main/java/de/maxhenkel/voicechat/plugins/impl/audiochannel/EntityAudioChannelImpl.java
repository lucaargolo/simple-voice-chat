...
public class EntityAudioChannelImpl extends AudioChannelImpl implements EntityAudioChannel {
    ...
    private void broadcast(PlayerSoundPacket packet) {
        if (entity.getEntity() instanceof net.minecraft.entity.Entity) {
            ...
            server.broadcast(ServerWorldUtils.getPlayersInRange((WorldServer) entity.worldObj, entity.getPositionEyes(1F), server.getBroadcastRange(distance), filter == null ? player -> true : player -> filter.test(new ServerPlayerImpl(player))), packet, null, null, null, SoundPacketEvent.SOURCE_PLUGIN);
        }
    }
